...
#include "sdk/claim.h"
#include "sdk/pio.h"
#include "sdk/pio_instructions.h"
...
// Not used, to avoid compiler warnings -Leo Jul 27, 2024
/*
...
static void _pio_add_program_at_offset(PIO pio, const pio_program_t *program, uint offset) {
    if (!_pio_can_add_program_at_offset(pio, program, offset)) {
        // Just trying to get this thing to compile - Leo Qu July 26, 2024
...
static int _pio_find_offset_for_program(PIO pio, const pio_program_t *program) {
    // Just trying to get this thing to compile - Leo Qu July 27, 2024
...
// These functions are not used in the course of Klipper-related functions - Leo Jul 27, 2024
